...
function pozicija(re,te) {
    ...
    var elemenat=document.getElementById('tabla').rows[parseInt(re)].cells[parseInt(te)];
...
document.getElementById('igra').innerHTML="Trenutno igra :"+player;
    ...
    x1 = elemenat.parentNode.rowIndex;
       ...
       else if(document.getElementById('tabla').rows[x].cells[y].getElementsByTagName('img')[0].src.split('/')[document.getElementById('tabla').rows[x].cells[y].getElementsByTagName('img')[0].src.split('/').length-1].split('_')[0]==player)
    {
        kliknutonafiguru = false;
        document.getElementById('tabla').rows[x].cells[y].style.opacity = "1";
...
potezilovac();
    ...
    if(r.includes('kralj')|| r.includes('kralji'))
    {
        for(var i=-1;i<2;i++)
        {
            for(var j=-1;j<2;j++)
            {
                if((x+i)*8+(y+j)<64 && (x+i)*8+(y+j)>=0 && document.getElementById('tabla').rows[x+i].cells[y+j].innerHTML=="" )
                {
potezi+=(x+i)+'-'+(y+j)+',';
                }
                   }
